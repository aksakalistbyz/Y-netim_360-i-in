...
function App() {
  ...
  const [registerData, setRegisterData] = useState({
    ...
    phoneNumber: '',
    isAdmin: false,
    flatCount: '10', // Yönetici için varsayılan
    flatId: ''       // Sakin için
  });
  ...
  const submitRegister = async () => {
    ...
    if (registerData.isAdmin && !registerData.flatCount) {
      alert("Kaç daire yöneteceğinizi belirtin.");
      return;
    }

    if (!registerData.isAdmin && !registerData.flatId) {
      alert("Lütfen dairenizi seçin.");
      return;
    }
  ...
  // Apartman koduna göre daireleri yükle (TOKEN OLMADAN)
  const loadFlatsForApartment = async (apartmentCode) => {
    if (!apartmentCode || apartmentCode.length < 5) {
      setAvailableFlats([]);
      return;
    }

    try {
      // Public endpoint kullan (token gerektirmez)
      const response = await fetch(`http://localhost:4000/api/flats/public/${apartmentCode}`);
      const data = await response.json();
      
      if (data.success) {
        setAvailableFlats(data.data.flats || []);
      } else {
        setAvailableFlats([]);
      }
    } catch (error) {
      console.error('Daire yükleme hatası:', error);
      setAvailableFlats([]);
    }
  };
  ...
  const payDue = async (feeId) => {
    try {
      if (currentUser?.role === 'admin') {
        // Yönetici direkt tahsil eder
        const response = await feeAPI.updateStatus(feeId, 'paid', 'Nakit');
        if (response.data.success) {
          await loadData();
          alert('Aidat tahsil edildi');
        }
      } else {
        // Sakin ödeme bildirimi yapar
        const response = await feeAPI.updateStatus(feeId, 'pending_approval', 'Banka Transferi');
        if (response.data.success) {
          await loadData();
          alert('Ödeme bildirimi gönderildi. Yönetici onayı bekleniyor.');
        }
      }
    } catch (error) {
      console.error('Ödeme hatası:', error);
      alert('Ödeme işlemi başarısız: ' + (error.response?.data?.message || error.message));
    }
  };
                  ...
                  <tr key={d.feeId}>
                    ...
                    <td>
                      <span className={`badge ${
                        d.status === 'paid' ? 'bg-green' : 
                        d.status === 'pending_approval' ? 'bg-yellow' : 
                        'bg-red'
                      }`}>
                        {d.status === 'paid' ? 'Ödendi' : 
                         d.status === 'pending_approval' ? 'Onay Bekliyor' :
                         'Ödenmedi'}
                      </span>
                    </td>
                    <td>
            ...
            <div className="modal-wrapper" onClick={closeModals}>
              <div className="modal-box" onClick={e => e.stopPropagation()}>
                ...
                {registerData.isAdmin && (
                  <input 
                    type="number" 
                    placeholder="Kaç daire yöneteceksiniz? (Örn: 10)" 
                    value={registerData.flatCount} 
                    onChange={e => setRegisterData({ ...registerData, flatCount: e.target.value })} 
                    disabled={loading}
                    min="1"
                  />
                )}
                ...
                {!registerData.isAdmin && (
                  <>
                    <input 
                      ...
                      onChange={e => {
                        setRegisterData({ ...registerData, apartmentNo: e.target.value });
                        loadFlatsForApartment(e.target.value);
                      }}
                      disabled={loading}
                    />
                    
                    <select
                      value={registerData.flatId}
                      onChange={e => setRegisterData({ ...registerData, flatId: e.target.value })}
                      disabled={loading || availableFlats.length === 0}
                      style={{ padding: '0.8rem', border: '1px solid #ddd', borderRadius: '0.5rem', marginBottom: '1rem' }}
                    >
